...
func (p *PbftNode) handlePrePrepare(content []byte) {
	p.RunningNode.PrintNode() // print the node information including NodeId, ShardId and Address
...
func (p *PbftNode) handlePrepare(content []byte) {
...
func (p *PbftNode) handleCommit(content []byte) {
	// decode the message
	...
	if cnt >= required_cnt && !p.isReply[string(cmsg.Digest)] {
		...
		if _, ok := p.requestPool[string(cmsg.Digest)]; !ok {
			...
			p.isReply[string(cmsg.Digest)] = true
			fmt.Println(string(cmsg.Digest))
...
func (p *PbftNode) handleRequestOldSeq(content []byte) {
	if p.view != p.NodeID {
		content = make([]byte, 0)
		return
	}
...
func (p *PbftNode) handleSendOldSeq(content []byte) {
